...
class Timezone(commands.Cog):
    ...
    async def timezone(self, interaction: nextcord.Interaction, country_or_city):
        try:
            response = requests.get(f"https://worldtimeapi.org/api/timezon/{country_or_city}")
            data = response.json()
        
            if response.status_code == 400 or "error" in data:
                await interaction.response.send_message("Cannot find. Please provide a valid country or city.")
            else:
