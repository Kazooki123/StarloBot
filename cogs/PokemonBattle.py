...
class PokemonBattle(commands.Cog):
    ...
    async def choose_pokemon(self, ctx, pokemon_name: str):
        ...
        if not pokemon:
            await ctx.send(f"☹️ {ctx.author.mention} **Invalid Pokèmon name! Try again.**", ephemeral=True)
            return
        ...
        if existing_pokemon:
            await ctx.send(
                f"🤦🏻 {ctx.author.mention} **You already have a Pokèmon!** Use `!release_pokemon` to choose a new one.")
            return
    ...
    @commands.command(name="pokebattle", help="Challenge another user to a Pokèmon battle!")
    ...
    @commands.command(name="release_pokemon", help="Release your current Pokèmon")
